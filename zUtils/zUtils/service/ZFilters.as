...
package zUtils.service{
	...
	public class ZFilters extends Sprite{
		...
		/**
		 * Убирает с объекта определённый фильтр. Если имя фильтра не передано, убираются все фильтры.
		 *
		 * @param target:DisplayObject - Объект, фильтры которого нужно очистить
		 * @param filter:String - имя фильтра, который нужно убрать. glow / dropShadow / grayscale / blur
		 *
		 */
		public static function clearFilters(target:DisplayObject, filter:String = null):void
		{
			...
			var filtersName:Object = {
										...
										grayscale:'flash.filters.ColorMatrixFilter',
										dropShadow:'flash.filters.DropShadowFilter',
		...
		public static function grayscale(
												target:DisplayObject,
												lumRed:Number = .2127,
												lumGreen:Number = .7152,
												lumBlue:Number = .0722
												):void{

			var filtersArr:Array = target.filters;

			var matrArr:Array = [lumRed, lumGreen, lumBlue, 0, 0,
								lumRed, lumGreen, lumBlue, 0, 0,
								lumRed, lumGreen, lumBlue, 0, 0,
								 0, 0, 0, 1, 0];
